...
export default function Dashboard() {
  const { assets, changelog } = useAssets();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Activos Inactivos</CardTitle>
            <AlertCircle className="h-4 w-4 text-warning" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-warning">{inactiveAssets}</div>
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Inversión en activos</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>Últimas modificaciones en el inventario</CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div key={entry.id} className="flex items-start gap-4 pb-4 border-b last:border-0">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-medium text-sm">{asset?.name || "Activo eliminado"}</span>
                      </div>
